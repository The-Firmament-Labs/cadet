import { requireOperatorApiSession } from "@/lib/auth";
import { createControlClient } from "@/lib/server";
import { sqlEscape } from "@/lib/sql";
import { apiError, apiNotFound } from "@/lib/api-response";

export async function GET(
  request: Request,
  context: { params: Promise<{ documentId: string }> },
) {
  const { unauthorized } = await requireOperatorApiSession(request);
  if (unauthorized) return unauthorized;

  const { documentId } = await context.params;
  try {
    const client = createControlClient();
    const rows = (await client.sql(
      `SELECT * FROM memory_document WHERE document_id = '${sqlEscape(documentId)}'`,
    )) as Record<string, unknown>[];
    if (rows.length === 0) return apiNotFound("Document not found");
    return Response.json({ ok: true, document: rows[0] });
  } catch (error) {
    return apiError(error, 500);
  }
}

export async function DELETE(
  request: Request,
  context: { params: Promise<{ documentId: string }> },
) {
  const { unauthorized } = await requireOperatorApiSession(request);
  if (unauthorized) return unauthorized;

  const { documentId } = await context.params;
  try {
    const client = createControlClient();
    await client.callReducer("delete_memory_document", [documentId]);
    return Response.json({ ok: true });
  } catch (error) {
    return apiError(error, 500);
  }
}
